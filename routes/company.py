...
def get_verified_company():
    """
    Checks that the logged-in user is a company AND that their
    account has been approved by admin. Returns the Company object
    if everything is fine, or returns None with an error response.
    """
...
def get_my_drives():
    ...
    return jsonify({
        "drives": [drive.to_dict() for drive in my_drives]
    }), 200
...
def update_application_status(application_id):
    ...
    drive_belongs_to_us = PlacementDrive.query.filter_by(
        id         = chosen_application.drive_id,
        company_id = this_company.id
    ).first()
...
def bulk_update_statuses():
    """
    Saves the status of multiple applications in one shot.
    The frontend sends a list like: [{ application_id: 1, status: 'shortlisted' }, ...]
    """
    this_company, error = get_verified_company()
    if error:
        return error
